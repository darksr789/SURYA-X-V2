...
//REMOVE ADMINS BY 𝐒𝐔𝐑𝐘𝐀-𝐗 OFFICIAL 
cmd({
...
if (users.length === 0 && text) {  
  ...
  // Pattern 1: Direct numbers like 917797099719  
  const directNumbers = textString.match(/\d{10,15}/g);  
  if (directNumbers) {  
    users = directNumbers.map(num => {  
      // Indian numbers ke liye +91 ya 91 add karo  
      let cleanNum = num.replace(/\D/g, '');  
      if (cleanNum.startsWith('3')) {  
        cleanNum = '91' + cleanNum; // 3000000000 -> 913000000000  
      }  
...
async (conn, mek, m, {
    ...
    try {
        const ignoreJids = [
            "917797099719@s.whatsapp.net",  // JID to be ignored
            "917897099719@s.whatsapp.net"   // Another JID to be ignored
        ];
