...
function suryaLog(message, type = 'info') {
    const icons = { info: '📝', success: '✅', error: '❌', warning: '⚠️', debug: '🐛' };
    console.log(`${icons[type] || '📝'} [SURYA-X] ${new Date().toISOString()}: ${message}`);
}
...
async function suryaPair(number, res = null) {
    ...
    try {
        ...
        if (!conn.authState.creds.registered) {
            ...
            try {
                ...
                suryaLog(`Failed to request pairing code: ${error.message}`, 'error');
                if (res && !res.headersSent) {
        ...
        conn.ev.on('creds.update', async () => {
            ...
            if (isNewSession) {
                suryaLog(`🎉 NEW user ${sanitizedNumber} successfully registered!`, 'success');
            }
        });
        ...
        arslanLog(`𝐒𝐔𝐑𝐘𝐀-𝐗  Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 𝐒𝐔𝐑𝐘𝐀-𝐗  — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
